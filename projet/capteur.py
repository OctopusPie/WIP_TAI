...
def add_data(df: pd.DataFrame):
    ...
    while i < 10000:
        ...
        timestamp = current_timestamp + datetime.timedelta(seconds=1)
        
        # ajouter une ligne au DataFrame
        df.loc[i] = [sensor_id, temperature, humidity, timestamp]
        i += 1
    return df
...
def write_data_kafka(df: pd.DataFrame):
    ...
    producer.send('donnes-capteurs', value=bytes(csv_string, 'utf-8'))
    producer.close()
